...
const publicArtworkNames = new Map([
  ['apple-glass-wallpaper', 'rose-glass'],
]);
...
test('clean artwork and Cursor Cat variant URLs rewrite to their built pages', () => {
  ...
  expect(config.rewrites).toEqual(expect.arrayContaining([
    expect.objectContaining({
      source: '/rose-glass/',
      destination: '/pages/apple-glass-wallpaper/index.html',
    }),
    expect.objectContaining({
      source: '/rose-glass/:path*',
      destination: '/pages/apple-glass-wallpaper/:path*',
    }),
    expect.objectContaining({
...
test('the Studio index only publishes clean artwork links', () => {
  ...
  expect(hrefs).not.toContain('/apple-glass-wallpaper/');
});
...
test('published social metadata uses clean Studio URLs', () => {
  ...
  for (const pageName of pageNames) {
    ...
    const publicName = publicArtworkNames.get(pageName) ?? pageName;
    expect(canonical, pageName).toBe(`https://studio.jiiwon.com/${publicName}/`);
