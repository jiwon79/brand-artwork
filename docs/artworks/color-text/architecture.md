...
누르는 동안 source는 `0.13초`에 질량 `1`을 공급한다. 이전처럼 `0.13초`마다 완성된 질량 `1` 입자를 갑자기 추가하지 않는다. 현재 source에 붙어 있는 공급 입자의 질량을 **매 프레임 조금씩** `0 → 1`로 채운다. 다 찬 입자만 source에서 떨어져 나가고, 같은 프레임부터 질량 `0`인 다음 공급 입자가 다시 자라기 시작한다.

```text
프레임 공급량 = 지난 시간 / 0.13초
현재 공급 입자 질량 += 프레임 공급량
질량이 1이 됨 → 낙하 입자로 전환
다음 공급 입자 → 질량 0부터 즉시 이어서 채움
Falloff 반경 = 기본 반경 × √현재 질량
```

반경 계산의 최소 질량도 두지 않는다. 따라서 새 공급 입자는 작은 원으로 갑자기 나타나는 대신 반경 `0`부터 연속적으로 커진다. 이미 다 찬 입자가 같은 위치에서 낙하를 시작하는 동안 다음 입자가 그 아래에서 자라므로 터치점과 물줄기 사이도 끊기지 않는다. 각 물리 프레임은 최대 `1/60초` 크기의 작은 단계로 나눠 다음 순서로 입자를 움직인다.
...
드래그 경로가 `8 px`을 넘으면 터치 시작 지연을 끝내지만, 이동 거리만으로 새 입자를 추가하지 않는다. 물의 총량은 오직 **누르고 있던 시간**에 비례한다. 아직 질량을 채우는 중인 `liquidSourceParticle`만 포인터를 즉시 따라간다.

```text
매 프레임:
  현재 공급 입자 위치 = 현재 포인터
  현재 공급 입자 질량 += 지난 시간 / 기본 공급 간격

질량이 1이 된 순간:
  이 입자는 현재 위치에서 분리되어 중력으로 낙하
  질량 0인 다음 공급 입자가 현재 포인터에서 시작
```

따라서 같은 시간 동안 손가락을 움직였다면 가로나 세로 이동 거리 때문에 물의 총량이 갑자기 늘지 않는다. 오래 머문 위치에서는 여러 입자가 차례로 분리되어 계속 흐르고, 빠르게 지나간 위치에는 실제로 그 시간 동안 공급된 양만 남는다. 손을 놓을 때도 채우던 입자의 현재 질량만 그대로 낙하시키므로 끝점에 질량 `1`짜리 큰 덩어리가 갑자기 추가되지 않는다.
...
- 현재 공급 입자는 매 프레임 질량 `0 → 1`로 채워 외곽이 시간 간격마다 튀는 현상을 막는다.
...
| `dripEmissionInterval` | `0.13 s` | 질량 `0 → 1`인 공급 입자 하나를 채우는 시간 |
