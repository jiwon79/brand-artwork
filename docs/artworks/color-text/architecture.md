...
누르고 있는 동안에는 `0.13초`마다 나이 `0`인 새 조각이 생겨 터치점에서 흐름이 계속 공급된다. 손을 떼면 새 조각은 더 만들지 않고, 머리는 기본 `0.35초` 동안 아래로 이동하면서 부드럽게 사라진다. 원래 위치에서 투명도만 줄이지 않으므로 영향권이 중앙부터 녹는 대신 아래로 빠져나가는 모양이 된다. 이미 나온 조각의 흐름용 `age`는 같은 속도로 증가하므로 앞부분이 갑자기 빨라지지 않고 그대로 아래로 흐른다.
...
- 손을 떼면 머리도 원래 위치에서 투명도만 줄이지 않고 중력 방향으로 내려가며 사라진다. 그래서 Falloff가 중앙부터 녹아 없어지지 않고 아래로 빠져나간다.
