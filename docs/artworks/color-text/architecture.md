...
글자를 고정 폭 칸 하나로만 잘라 읽으면 `W`처럼 폭이 넓은 글자가 이동하거나 회전할 때 칸 밖의 픽셀이 사라질 수 있다. 현재 구현은 각 글자의 실제 좌우 폭을 `glyphMetadataTexture`에 저장하고, 출력 픽셀마다 가장 가까운 글자 칸과 양옆 칸까지 조사한다. 공백 칸은 메타데이터로 제외한다. 따라서 `WAY`의 `W` 왼쪽이나 Active 화면의 움직이는 획이 고정 셀 경계에서 잘리지 않는다.
