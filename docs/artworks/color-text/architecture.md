...
누르는 동안 source는 `0.13초`에 질량 `1`을 공급한다. 첫 공급 입자만 **매 프레임 조금씩** `0 → 1`로 채워 처음 터치가 자연스럽게 나타나게 한다. 첫 입자가 다 찬 뒤에는 그 완성된 표면을 포인터에 계속 붙여 둔다. 다음 질량 `1`이 시간 예산에 모두 준비된 순간에만 같은 크기의 새 source 입자와 교대하고, 이전 입자는 그때부터 낙하한다.
...
`dripEmitter`는 초당 비율 `13`의 지수 보간으로 포인터를 따라간다. 이 좌표는 앞으로 태어날 입자의 위치에만 사용한다. 이미 생성된 입자의 위치는 포인터가 아니라 자신의 속도와 주변 입자의 힘으로 바뀐다.
...
드래그 경로가 `8 px`을 넘으면 터치 시작 지연을 끝낸다. 완성된 `liquidSourceParticle`은 포인터를 즉시 따라가므로 새 위치에 빈 프레임이 생기지 않는다. 물의 총량은 여전히 **누르고 있던 시간**에 비례한다.

```text
매 프레임:
  완성된 source 표면 위치 = 현재 포인터
  다음 교대용 질량 += 지난 시간 / 기본 공급 간격

드래그 거리가 8 px을 넘은 구간:
  교대용으로 쌓인 질량 일부를 0.04~0.10의 작은 입자로 분할
  긴 포인터 이벤트 구간은 8 px 간격으로 다시 보간
  분할에 사용한 질량은 교대 예산에서 차감
```

따라서 긴 드래그 이벤트도 큰 점 몇 개가 아니라 서로 겹치는 작은 경로 입자로 이어진다. 이 입자는 새로운 물을 추가한 것이 아니라 다음 source 교대를 위해 이미 쌓인 양을 옮긴 것이므로, 좌우 이동 거리가 길다고 전체 Falloff 질량이 다시 과도하게 커지지 않는다. 예산이 `0.04`보다 작으면 경로 입자를 만들지 않고 완성된 source 표면만 포인터를 따라간다.

이미 source를 떠나 충분히 오래 떨어진 입자는 새 포인터로 이동시키지 않는다. 그래서 이전 위치의 물은 그 자리에서 계속 낙하하고, 포인터 가까이에 있던 가장 최신 공급분만 새 위치를 따라간다. 포인터의 가로 속도가 입자에 전달되는 비율도 `4%`로 제한해 빠른 좌우 드래그가 Falloff를 옆으로 과도하게 벌리지 않도록 한다.
...
| `dripFollowEase` | `13` | 포인터 속도를 부드럽게 측정해 새 입자의 초기 속도에 반영하는 비율 |
