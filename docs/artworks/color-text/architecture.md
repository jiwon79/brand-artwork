...
손가락을 처음 누르면 첫 입자는 강도 `0`에 가깝게 시작하고 약 `0.36초` 동안 자연스럽게 나타난다.

```text
birthEnergy = 1 - exp(-입자 나이 / 0.36초)
실제 입자 강도 = birthEnergy²
```

따라서 완성된 실루엣이 첫 프레임에 갑자기 튀어나오지 않는다. 작품 좌표로 `8 px` 이상 드래그한 뒤 새로 생기는 입자는 이 대기를 건너뛴다. 손을 빨리 떼면 아직 증가 중이던 입자의 강도를 그 순간 값으로 고정하므로, 릴리즈 뒤에 물의 양이 뒤늦게 커지지 않는다.

#### 5.3.2 모든 물 입자는 같은 중력과 점성 규칙으로 움직인다

누르는 동안 source는 기본 `0.13초`마다 새 입자를 만든다. 별도의 안정된 머리나 릴리즈 전용 덩어리는 없다. 각 프레임은 최대 `1/60초` 크기의 작은 단계로 나눠 다음 순서로 입자를 움직인다.
...
입자가 `32개`를 넘으려 하면 가장 가까운 두 입자를 하나로 합친다. 오래된 입자를 그냥 버리지 않고 질량과 운동량을 보존한다.

```text
합친 질량 = 질량 A + 질량 B
합친 위치 = 두 위치의 질량 가중 평균
합친 속도 = 두 속도의 질량 가중 평균
Falloff 반경 = 기본 반경 × √합친 질량
```

그래서 오랫동안 누르거나 빠르게 드래그해도 물이 중간에서 증발하지 않는다. 합쳐진 입자는 더 많은 물을 가진 하나의 큰 입자가 되고 계속 흐른다.
...
드래그 경로는 `8 px`마다 다시 표본을 잡지만, 그때마다 같은 크기의 입자를 새로 만들지는 않는다. 물의 총량은 이동 거리가 아니라 **누르고 있던 시간**에 비례하도록 `dripMassBudget`에 쌓인다.

```text
이번 프레임에 공급된 물의 양 = 지난 시간 / 기본 방출 간격

정지 상태:
  예산이 1 이상이면 질량 1인 입자 방출

드래그 상태:
  8 px마다 현재까지 쌓인 예산만 사용
  한 경로 입자의 질량은 최소 0.08, 최대 0.35
  예산이 0.08보다 작으면 새 입자를 만들지 않고
  아직 source를 막 떠나지 않은 가장 최신 입자만 새 포인터로 이동
```

따라서 같은 시간 동안 손가락을 움직였다면 가로나 세로 이동 거리 때문에 물의 총량이 갑자기 늘지 않는다. 빠른 드래그에서는 작은 질량들이 경로를 연결하고, 정지하면 같은 예산이 모여 기본 크기의 입자가 된다. 손을 놓는 끝점도 이 예산 규칙을 그대로 사용하므로 마지막 위치에 질량 `1`짜리 큰 덩어리가 갑자기 추가되지 않는다.
...
| `dripFollowEase` | `13` | 누른 채 드래그할 때 source가 포인터를 따라가는 속도 |
| `dripEmissionInterval` | `0.13 s` | 누르는 동안 새 물 입자를 만드는 간격 |
