...
GPU의 각 출력 픽셀은 안정된 머리와 최대 `32개` 방출 조각을 차례로 계산한다. 각 조각의 나이로 흐르기 전 Falloff 위치를 역산한다. 조각은 생성점에서 아래로 약 `34 px` 진행하기 전까지 `flowGate`로 억제한다. 따라서 머리 가까이에서는 새 조각의 생성·이동·교체가 외곽선을 흔들지 않고, 그 아래에서만 조각들이 흐름을 잇는다.

그다음 가장 강한 값 하나만 딱 고르는 대신 가장 강한 값과 두 번째 값을 찾는다. 두 값의 차이가 작아 승자가 바뀌기 쉬운 구간에서는 둘의 평균 쪽으로 부드럽게 이동하고, 차이가 충분히 크면 강한 값만 사용한다. 기본 전환 폭은 `0.08`이다. 모든 조각을 더하지 않기 때문에 실루엣이 불필요하게 넓어지지 않으면서도 `max()`의 갑작스러운 승자 교체가 사라진다.

```text
각 조각 i에 대해 qᵢ = flowInverse(p, originᵢ, ageᵢ)
머리 값 H(p) = falloff(p, 현재 터치 위치) × 머리 강도
조각 값 Pᵢ(p) = falloff(qᵢ, originᵢ) × flowGate × birthFade
이어진 Falloff 값(p) = smoothTopTwo(H(p), P₁(p), ..., P₃₂(p))
...
누르고 있는 동안에는 `0.13초`마다 나이 `0`인 새 조각이 생겨 터치점에서 흐름이 계속 공급된다. 손을 떼면 머리는 기본 `0.35초` 동안 부드럽게 사라지고 새 조각은 더 만들지 않는다. 이미 나온 조각의 흐름용 `age`는 같은 속도로 증가하므로 앞부분이 갑자기 빨라지지 않고 그대로 아래로 흐른다.
...
- 흐르는 조각은 생성점 아래 약 `34 px`부터만 나타나도록 해 머리 영역과 분리한다.
- 가장 강한 두 조각의 값이 비슷할 때 기본 폭 `0.08`로 부드럽게 교차시켜 `max()`의 승자 교체를 감춘다.
