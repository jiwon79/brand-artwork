...
### 5.8 액체 아래의 배경 글자를 잠깐 아래로 민다

실루엣을 만든 뒤 `finalMaterial`은 배경 글자를 합성한다. 이때 원래 위치의 글자 마스크를 바로 읽지 않고, 해당 출력 픽셀보다 위쪽 `18 px` 범위에서 액체 field가 있는지 세 번 확인한다.

```text
push = 위쪽 18 px 안에서 읽은 가장 강한 액체 field
이동량 = smoothstep(낮은 기준, 높은 기준, push) × 9 px
화면의 글자 픽셀 = 원래 글자 마스크(현재 위치 + 이동량)
```

WebGL 작품 좌표에서 위쪽의 원본 픽셀을 읽어 현재 위치에 놓으면 화면에서는 글자가 아래로 이동한 것처럼 보인다. 액체가 강할수록 최대 `9 px`까지 밀리고, field가 지나가면 `push`가 다시 `0`이 되어 원래 자리로 돌아간다.

중요하게도 이 변형은 **마지막 화면 합성에서 보이는 배경 글자에만** 적용한다. `interactionFieldMaterial`이 액체를 만들 때 쓰는 원래 글자 마스크는 움직이지 않는다. 따라서 글자 밀림이 다시 액체의 입력을 바꾸는 피드백 고리는 생기지 않고, 기존 Metaball 실루엣도 유지된다.
...
| `surfaceFieldTarget` | 최종으로 매끄러워진 높이 지도 | 외곽선 추출과 배경 글자 밀림 계산 |
...
function animate(now) {
  ...
  // 6. 기준 높이에서 경계를 꺼내고 액체 아래 글자를 밀어 화면에 그린다.
...
| `textPushDistance` | `9 px` | 액체 아래 배경 글자가 내려가는 최대 거리 |
| `textPushReach` | `18 px` | 각 글자 출력 픽셀이 위쪽 액체를 확인하는 범위 |
...
일반 화면에서는 `lil-gui` 패널이 보이며, `터치 드립`, `텍스트 밀림`, `광원 / Falloff`, `액체 실루엣`, `색상`, `고급 설정` 폴더로 나뉜다. `터치 드립` 폴더에서 중력·폭·수명과 함께 시작 시간·드래그 추종·방출 간격을 조절할 수 있다. `텍스트 밀림`에서는 최대 하강 거리와 액체 아래의 영향 범위를 바꿀 수 있다. 키보드의 `g`를 누르면 패널을 숨기거나 다시 열 수 있다. 자동 비교용 QA 모드에서는 이미지 비교를 가리지 않도록 패널이 처음부터 숨겨진다. 실루엣을 조절할 때는 한 번에 하나의 값을 바꾸고 Y 교차부를 먼저 확인하는 것이 안전하다.
