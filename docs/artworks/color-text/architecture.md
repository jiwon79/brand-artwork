...
`glyphSpringMaterial`은 각 글자의 실제 잉크 중심·폭·높이를 사용한다. 움직이고 회전한 글자의 중심과 상하·좌우·대각선까지 모두 `9곳`에서 `surfaceFieldTarget`을 읽는다. 한 곳이라도 액체 실루엣 안에 들어오면 `contact`가 커지므로 글자 중심을 비껴간 접촉도 감지할 수 있다.

하강에는 아홉 곳 중 가장 강한 접촉값을 사용한다. 회전에는 왼쪽 세 곳과 오른쪽 세 곳을 따로 묶은 뒤 그 차이를 사용한다.
