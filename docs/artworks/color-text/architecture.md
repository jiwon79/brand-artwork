...
### 5.3 시간은 픽셀 이미지가 아니라 방출 구간으로 기억한다

현재 코드는 이전 활성 픽셀 이미지를 저장하는 `temporalMaterial`이나 ping-pong buffer를 사용하지 않는다. 대신 “누른 뒤 몇 초 동안 Falloff를 방출했는가”를 `dripEmissionDuration`이라는 숫자로 기억한다. 클릭하거나 터치한 순간에는 위치를 `dripAnchor`에 복사하지만 **활성화된 글자 픽셀 이미지는 저장하지 않는다.**

#### 5.3.1 첫 터치는 강도가 천천히 올라간다

터치 시작 시 `dripEnergy`를 `0`으로 두고, 매 프레임 목표값 `1`에 조금씩 가까워지게 한다.

```text
dripEnergy ← mix(dripEnergy, 1, 1 - exp(-프레임 시간 / 0.18초))
실제 입력 강도 = dripEnergy²
```

따라서 첫 프레임에는 거의 보이지 않는 작은 활성값만 있고 약 `0.18초`에 걸쳐 에너지가 올라간다. 이 값을 한 번 제곱해 낮은 초반 값을 더 작게 만든 뒤 Metaball에 넣는다. 주변 누적 때문에 약한 입력도 크게 합쳐지는 현상을 보정해 첫 터치에 완성된 실루엣이 갑자기 나타나는 느낌을 줄인다.

#### 5.3.2 서로 다른 나이의 Falloff가 이어진 흐름을 만든다

Falloff 하나만 아래로 옮기면 타원 전체가 낙하하는 것처럼 보인다. 그래서 누르고 있는 동안 서로 다른 나이의 Falloff를 계속 방출한다고 생각한다. 방금 나온 것은 나이 `0`, 먼저 나온 것은 `0.8초`, 더 먼저 나온 것은 `1.6초`처럼 취급한다. 어린 영역은 터치점에 남고 오래된 영역일수록 중력 때문에 더 아래에 있다.
...
코드가 실제 구슬이나 입자 이미지를 저장하는 것은 아니다. GPU의 각 출력 픽셀에서 현재 방출 구간을 대표하는 `18개`의 나이를 고르고, 각 나이마다 흐르기 전 Falloff의 위치를 역산한다. `18개` 결과 중 가장 강한 값을 사용하므로 떨어진 타원 여러 개가 아니라 이어진 하나의 영향 영역이 된다.

```text
각 나이 a에 대해 q(a) = flowInverse(p, dripAnchor, a)
이어진 Falloff 값(p) = max(falloff(q(a), dripAnchor))
...
누르고 있는 동안에는 방출 구간의 가장 어린 나이가 항상 `0`이라 터치점에서 계속 새 흐름이 시작된다. 손을 떼면 새 나이 `0`을 만들지 않고 가장 어린 나이도 계속 증가한다. 그래서 위쪽 꼬리부터 터치점과 분리되고, 이미 나온 앞부분은 계속 아래로 흐른다. 각 방출 영역은 최대 `4초` 동안만 살아 있다.

#### 5.3.3 드래그하면 방출 위치와 시간 구간을 함께 갱신한다

드래그 중에 단순히 `dripAnchor`만 옮기면 이미 아래로 내려간 긴 흐름까지 옆으로 통째로 이동한다. 이를 막기 위해 앵커가 실제로 이동한 작품 픽셀 거리만큼 방출 구간의 오래된 끝을 줄인다. 기본값은 `1 px` 이동할 때 오래된 방출 시간 `0.012초`를 제거하는 것이다. 빠르게 이동할수록 기존 긴 흐름은 빨리 짧아지고, 새 위치에서는 나이 `0`인 영역부터 다시 자란다. 앵커 자체도 초당 비율 `13`의 지수 보간으로 따라가므로 순간이동처럼 끊기지 않는다.
...
function animate(now) {
  // 1. 누르는 중이면 방출 위치·강도·시간 구간을 갱신한다.
  updateDripEmitter();
...
| `dripMoveClear` | `0.012 s/px` | 이동 거리마다 제거할 오래된 방출 시간 |
...
일반 화면에서는 `lil-gui` 패널이 보이며, `터치 드립`, `광원 / Falloff`, `액체 실루엣`, `색상`, `고급 설정` 폴더로 나뉜다. `터치 드립` 폴더에서 중력·폭·수명과 함께 시작 시간·드래그 추종·이전 흐름 제거량을 조절할 수 있다. 키보드의 `g`를 누르면 패널을 숨기거나 다시 열 수 있다. 자동 비교용 QA 모드에서는 이미지 비교를 가리지 않도록 패널이 처음부터 숨겨진다. 실루엣을 조절할 때는 한 번에 하나의 값을 바꾸고 Y 교차부를 먼저 확인하는 것이 안전하다.
