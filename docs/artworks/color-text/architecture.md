...
손가락을 처음 누른 뒤 아직 드래그하지 않았다면 그 자리에 공급되는 조각은 나이 `0`에서 강도 `0`으로 시작하고 약 `0.36초`에 걸쳐 자연스럽게 나타난다.
...
`LOR`처럼 실루엣의 윗 경계에 걸친 글자는 내부에 완전히 잠긴 글자보다 접촉값이 불안정하다. 정지 터치에서도 새 Falloff 조각은 `0.13초`마다 생기고 기존 조각은 계속 아래로 흐른다. 이때 81개 검사 지점 중 가장 강한 값을 쓰는 `contact`가 경계 부근에서 오르내리면 목표 하강량도 `0 px`과 최대 `10 px` 사이에서 계속 바뀐다.

현재 하강 스프링의 강성은 `58`, 감쇠는 `12`다. 질량을 `1`로 보는 이 계산의 임계 감쇠는 대략 `2 × √58 = 15.2`이므로 현재 값은 약한 언더댐핑 상태다. 경계 접촉이 바뀔 때 남아 있던 속도가 바로 사라지지 않아 글자가 목표 위치를 지나쳤다가 돌아온다. 변형된 글자 마스크가 다음 프레임의 실루엣 입력에도 다시 사용되므로, 경계에 있는 글자의 이동과 접촉 변화가 한 프레임 간격으로 서로 영향을 주는 피드백도 생긴다.

정지 터치 분리 실험에서는 다음 결과를 확인했다.

- 최대 하강 거리와 최대 회전을 모두 `0`으로 두면 `LOR`이 고정됐다.
- 하강 `10 px`, 회전 `0°`에서는 위치 변화가 다시 나타났다.
- 하강 `0 px`, 회전 `9°`에서는 변화가 거의 없었다.
- 하강 감쇠만 `12`에서 `22`로 높이면 같은 경계 접촉에서도 위치가 안정됐다.

따라서 이 떨림의 주원인은 색 순환이나 회전이 아니라 **경계에서 변하는 접촉 목표값과 언더댐핑된 세로 스프링의 결합**이다. 해결할 때는 하강 감쇠를 임계 감쇠 근처로 높이거나, 접촉값에 시간 저역 통과 필터 또는 켜짐·꺼짐 임계값이 다른 히스테리시스를 두는 방법을 비교할 수 있다. 이 진단 단계에서는 움직임의 성격을 바꾸지 않기 위해 감쇠 기본값은 수정하지 않았다.
