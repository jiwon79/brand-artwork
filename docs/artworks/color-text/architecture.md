...
드래그가 `8 px`을 넘을 때마다 타이머를 기다리지 않고 현재 포인터에 입자를 즉시 추가한다. 손을 놓는 위치가 마지막 입자에서 `6 px`보다 멀거나 마지막 방출 뒤 시간이 조금 지났다면, 끝점에도 마지막 입자를 추가한다. 빠른 드래그에서도 물줄기의 끝이 빠지지 않지만 이전 위치의 물이 새 포인터로 통째로 이동하지는 않는다.
