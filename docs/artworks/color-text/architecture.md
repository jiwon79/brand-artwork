...
누르고 있는 동안의 머리는 물 덩어리가 아니라 **물을 계속 공급하는 출구**다. `0.13초`마다 나이 `0`인 새 조각을 만들어 터치점 아래의 흐름을 유지한다. 손을 떼면 출구에서 새 조각을 만드는 일만 멈춘다. 출구에 있던 마지막 Falloff와 이미 방출된 조각은 삭제하거나 투명하게 만들지 않고 같은 중력 계산으로 계속 내려간다.

릴리즈 순간의 출구 위치에서 시작하는 `releaseRearGate`는 마지막 물 덩어리의 윗면을 따라 중력 방향으로 이동한다. 이 경계보다 위는 새 물이 공급되지 않은 빈 공간이고, 아래는 이전에 나온 물이다. 따라서 실루엣을 인위적으로 닦아내지 않아도 흐름의 꼬리가 위에서 끊겨 아래로 빠져나간다. 조각의 흐름용 `age`는 계속 실시간으로 증가하고, 소멸용 `lifeAge`만 기본 `2배`로 증가해 너무 오래 남는 조각을 정리한다. 마지막 물 덩어리와 뒤쪽 경계는 최대 `1.5 × 화면 높이`까지 계속 낙하할 수 있으므로 화면 가운데에 멈추지 않는다.
...
- 손을 떼면 머리를 삭제하지 않고 마지막 물 덩어리로 바꿔 중력 방향으로 보낸다. 새 공급이 끊긴 경계도 함께 내려가므로 흐름 전체가 화면 아래로 빠져나간다.
