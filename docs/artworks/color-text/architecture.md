...
### 5.3.3 두 확장 인터랙션은 서로 섞이지 않는다

`interactionFieldMaterial`은 시간 기억 다음에 실행되며 `classic`, `viscous`, `thermal` 중 하나만 계산한다. 따라서 점성 꼬리와 체류 열꽃은 같은 프레임에 동시에 적용되지 않는다.
...
| `interactionMode` | `viscous` | 기본 작품, 점성 꼬리, 체류 열꽃 중 한 모드 선택 |
