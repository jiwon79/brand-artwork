...
누르고 있는 동안에는 방출 구간의 가장 어린 나이가 항상 `0`이라 터치점에서 계속 새 흐름이 시작된다. 손을 떼면 새 나이 `0`을 만들지 않고 가장 어린 나이도 계속 증가한다. 그래서 위쪽 꼬리부터 터치점과 분리되고, 이미 나온 앞부분은 계속 아래로 흐른다. 각 방출 영역은 최대 `4초` 동안만 살아 있다. 짧게 탭해도 최소 `0.18초` 길이의 방출 구간을 만들어 한 번의 드립은 보인다.
