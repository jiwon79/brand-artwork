...
생성 구간의 작은 크기 차이는 런타임 calibration의 `displayScales`로 보정한다. Top 접점에서는 생성 프레임의 실루엣 면적 증가를 상쇄하도록 `1 → 0.986`으로 보정한 뒤 Left까지 다시 `1`로 완만하게 돌아온다. Left 접점에서는 다음 영상의 첫 프레임을 `0.996`으로 맞춘 뒤 아래 반원에서 다시 `1`로 돌아온다. 숫자 자체보다 보정 후 접점 양쪽의 실루엣 면적과 체감 크기가 연속적인지가 중요하다.

배경 노이즈가 flood fill 임계값보다 어두워 남는 단일 프레임은 임계값을 전역으로 낮추지 않는다. 흰 털까지 배경으로 오인할 수 있기 때문이다. 대신 앞뒤 프레임의 alpha 합집합을 4px 확장한 envelope 밖의 alpha만 제거한다. 현재 아래 반원 초반의 frame 063에 이 처리를 적용한다.
