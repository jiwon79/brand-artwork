...
3. 고양이 이미지가 표현하는 방향과 눈 기준점에서 pointer로 향하는 방향 사이의 각도 차이를 구한다. 이때 0°와 360°가 같은 방향이라는 점을 반영해, 원을 따라 가장 짧은 차이를 사용한다. 예를 들어 359°와 1°의 차이는 358°가 아니라 2°다.
4. 방향 차이가 가장 작은 frame을 target으로 선택한다.
