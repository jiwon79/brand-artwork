...
#### 연결한 외곽선의 시작점과 진행 방향을 고정한다

Marching squares가 만든 선분의 끝점들이 곧바로 cage 기둥이 되는 것은 아니다. 격자와 외곽선이 만난 위치에 따라 끝점 간격이 들쭉날쭉하기 때문이다.

CPU는 먼저 선분들을 연결해 하나의 긴 외곽선 경로를 만든다. 외곽선이 여러 개라면 면적이 가장 큰 것을 고른다. 이 경로에 바로 번호를 붙이면 두 문제가 생긴다.

- 외곽선을 어느 방향으로 도느냐에 따라 `1, 2, 3, ...`의 순서가 뒤집힐 수 있다.
- 닫힌 외곽선에는 본래 시작점이 없으므로, 매번 다른 점이 0번이 될 수 있다.

왼쪽 기둥과 오른쪽 기둥이 같은 번호로 묶이지 않게 하려면 먼저 **한 바퀴를 도는 방향**과 **0번이 시작되는 seam**을 고정해야 한다. `orderContour()`는 다음 규칙을 사용한다.

1. 외곽선의 signed area를 계산하고, 점 목록이 항상 같은 방향으로 돌도록 필요하면 순서를 뒤집는다.
2. 처음 계산할 때는 가장 오른쪽에 있으면서 세로 중심에 가까운 점을 0번 seam으로 고른다.
3. 다음 계산부터는 이전 seam 위치와 가장 가까운 새 외곽선 점을 0번으로 고른다.

세 번째 규칙 때문에 모양이 프레임마다 조금씩 바뀌어도 0번이 갑자기 다른 곳으로 점프하지 않는다. 시작점과 진행 방향이 고정되면 늘어난 외곽선의 오른쪽 0번은 원의 오른쪽 0번과 대응하고, 외곽선을 절반쯤 돈 반대편 기둥은 원에서도 대략 `N/2`번과 대응한다. 따라서 늘어난 외곽선의 왼쪽과 원의 오른쪽이 한 쌍이 되는 식의 뒤바뀜을 막을 수 있다.

정확히 말하면 이 구현은 각 기둥의 법선 각도나 회전량을 비교해 가장 비슷한 기둥을 검색하는 방식은 아니다. **같은 seam에서 출발해 같은 방향으로 돈 둘레 비율**을 대응시키는 방식이다. 연속적으로 조금씩 변하는 현재 artwork에는 안정적이지만, 한 프레임 만에 모양이 크게 뒤집히거나 외곽선 구조가 달라지면 같은 물질점을 완벽하게 추적하지는 못한다.

#### 늘어난 외곽선 위에 기둥을 같은 간격으로 다시 세운다

방향과 seam을 정한 다음에는 경로의 전체 둘레 `L`을 잰다. 기둥을 `N`개 세운다면 `k`번째 기둥이 놓일 목표 거리는 다음과 같다.

```text
늘어난 외곽선의 k번째 거리 = L * k / N
```

CPU는 seam에서 출발해 연결된 선분의 길이를 차례로 더하다가 이 목표 거리에 도착한 위치를 선형 보간한다. 이렇게 같은 둘레 간격으로 새로 찍은 점들이 실제 cage 기둥이다. 현재 설정에서는 `N`, 즉 `specBoundarySamples`로 16~64개를 사용한다.
...
#### 원래 원은 SDF 없이 수식으로 나눈다

Canonical shape는 반지름 1인 원이라는 사실을 이미 알고 있으므로 다시 SDF를 샘플링해 외곽선을 찾을 필요가 없다. 원의 `k`번째 기둥은 각도와 삼각함수로 바로 계산한다.

```text
theta_k = 2 * PI * k / N

circle_k.x = cos(theta_k)
circle_k.y = sin(theta_k)
```

예를 들어 `N = 8`이면 각도 간격은 `360도 / 8 = 45도`다. `theta`를 라디안으로 나타내면 원의 호 길이는 `s = r * theta`다. 반지름 `r`이 1이므로 `s = theta`가 되어, 같은 각도 간격이 곧 같은 둘레 간격이다.

```text
늘어난 외곽선: seam부터 전체 둘레의 k/N 지점
                         <->
원래 단위 원: 각도가 2 * PI * k/N인 지점
```

그래서 늘어난 외곽선의 `k`번 기둥과 원의 `k`번 기둥이 한 쌍이 된다. 원에는 SDF나 Marching squares가 필요 없고, 늘어난 외곽선에만 그것들이 필요하다.
...
4. 외곽선의 진행 방향과 0번 seam을 고정한다.
5. 외곽선을 따라 같은 간격으로 `specBoundarySamples`개의 기둥을 다시 배치한다.
6. 원래 원에는 `cos`, `sin` 수식으로 같은 개수의 기둥을 배치한다.
7. 같은 index의 두 기둥을 한 쌍으로 저장한다.
