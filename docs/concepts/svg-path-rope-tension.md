...
| `T_i` | 0–1 | 먼 구간에도 남겨 둘 `i`번째 sample의 최소 전달분 |
| `D_i` | 0–1 | local tension과 최소 전달분을 합친 목표 장력 |
| `S_i` | 0–1 | 시간에 따라 목표 장력에 도달한 비율 |
...
`L_i`는 수학적으로 0이 되지는 않지만, anchor에서 충분히 멀어지면 화면에서는 움직임을 알아보기 어려울 만큼 작아진다. 그러면 가까운 구간은 움직이고 먼 구간은 정지한 것처럼 보여 그 경계에서 선의 움직임이 끊긴 듯한 인상이 생길 수 있다. 이를 피하려면 작은 최소 전달분 `T_i`를 섞어 먼 sample도 조금은 따라오게 할 수 있다.

`T_i`는 모든 sample에 같은 상수를 사용할 수도 있고, sample `i`의 거리나 당김 진행도에 따라 다르게 계산할 수도 있다. 값이 너무 작으면 정지 구간이 다시 눈에 띄고, 너무 크면 먼 부분까지 같은 양으로 움직여 형상 전체가 통째로 미끄러져 보인다.
...
`D_i = L_i + (1 - L_i) × T_i`에서 anchor 근처는 `L_i ≈ 1`이므로 `T_i`의 영향이 거의 없다. 반대로 먼 지점은 `L_i ≈ 0`이므로 `D_i ≈ T_i`가 되어 최소 전달량이 남는다. 따라서 `T_i`는 가까운 구간의 강한 당김은 유지하면서 먼 구간만 완전히 멈추지 않게 하는 하한 역할을 한다.
...
| 단계 | 구현 |
| --- | --- |
| SVG contour sample | [`geometry.ts`의 `loadLineAsset()`](../../pages/body-echo/geometry.ts) |
| contour 내부 edge와 virtual joint | [`geometry.ts`의 `buildLineGraph()`](../../pages/body-echo/geometry.ts) |
| anchor와 graph distance cache | [`runtime.ts`의 `ropeFieldFor()`](../../pages/body-echo/runtime.ts) |
| 지수 감쇠와 최종 위치 | [`runtime.ts`의 `gatheredLinePosition()`](../../pages/body-echo/runtime.ts) |

Body Echo는 hold를 시작할 때 각 인체의 anchor와 `graphDistances`를 만든다. 손가락을 움직이는 동안 anchor는 유지하고 목표점 `contactOrigin`만 바꾼다. 따라서 처음 잡은 strand를 놓지 않고 끌고 가는 동작이 된다.
