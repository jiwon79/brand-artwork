...
두 기둥 사이가 `P`에서 넓게 벌어져 보일수록 `t_i`가 커진다. 실제 shader는 `atan()`으로 각도를 구한 뒤 다시 `tan()`을 호출하지 않는다. 두 벡터의 내적과 외적을 이용해 같은 값을 바로 계산한다.
...
이 식은 `tan(alpha_i / 2)`와 같으며, 각도를 명시적으로 구하지 않아도 된다.
