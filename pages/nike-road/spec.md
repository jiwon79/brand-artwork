...
- **공 물리**: 
  - 위치 (x: 215, y: -20), 속도, 반지름(7px), 플랫폼 착지 상태 추적
  - 초기 x 위치는 215 (오른쪽에서 시작)
  - `resetPhysics()` 함수는 x: 215로 리셋하여 초기 위치와 일치
...
- **#layout** - 전체 레이아웃 래퍼
  - **#stage** - 캔버스와 하늘 요소를 포함하는 스테이지 컨테이너
    - **#sky** - 하늘 배경 요소
    - **#scene** - Canvas 렌더링 컨테이너 (720×405px)
  - **#controls** - 하단 버튼 그룹
    - **#btn-play** - 재생/일시정지
...
- **#stage**: 상대 위치 컨테이너 (position: relative, 720×405px, box-shadow)
