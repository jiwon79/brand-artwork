...
- The active artwork contains one center image and two 49-pose semicircular arcs. There are no `CENTER → LEFT` or `CENTER → RIGHT` paths.
...
- Each visible pose owns a normalized pointer position: one center pointer, 49 upper-arc pointers, and 47 lower-arc intermediates. Shared left/right endpoints appear only once, for 97 pointer-owned images in total.
...
- Generate the upper and lower semicircles separately with LTX 2.5 First/Last Frame at 576 × 1024, 4 seconds, and 24 fps.
