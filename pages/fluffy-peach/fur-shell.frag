...
void main() {
  ...
  float coverage = smoothstep(0.27 + 0.16 * uLayer, 0.78, mix(broad, fine, 0.48));
  float falloff = pow(1.0 - uLayer, 1.25);
  float rim = pow(vSilhouette, 1.35);
  float alpha = (0.07 + 0.20 * rim) * falloff * coverage;
  vec3 color = mix(vFurColor, vec3(0.98, 0.87, 0.94), 0.08 + 0.18 * uLayer);
