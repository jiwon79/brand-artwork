...
const lights: Light[] = [
  {
    ...
    target: new THREE.Vector2(0.5, 0.595),
...
const fragmentShader = `
  ...
  vec3 backgroundAt(vec2 uv) {
    ...
    vec3 base = vec3(0.068, 0.0003, 0.0010);
    base += vec3(0.29, 0.0015, 0.005) * centerBloom * 0.72;
    base += vec3(0.42, 0.001, 0.005) * curtain * vignette * uAmbientRays;
  ...
  vec3 rawLight(
    ...
    vec3 spectralEdge = vec3(1.0, 0.18, 0.005) * edgeWarm * 1.34;
    spectralEdge += vec3(0.005, 0.24, 1.0) * edgeCool * 1.52;

    float angleMix = smoothstep(-spread * 0.82, spread * 0.82, signedAngle);
    vec3 prismTint = mix(vec3(1.0, 0.34, 0.015), vec3(0.02, 0.34, 1.0), angleMix);
    float prismAmount = body * smoothstep(0.22, 0.96, abs(signedAngle) / max(spread, 0.001));
    vec3 volume = mix(lightColor, prismTint, prismAmount * 0.26) * body;
    ...
    return (volume + spectralEdge) * lightIntensity * uIntensity + source * lightIntensity;
  }
  ...
  void main() {
    ...
    for (int lightIndex = 0; lightIndex < MAX_LIGHTS; lightIndex++) {
      ...
      float outgoing = lightIndex == 0 ? 0.42 : 0.035;
    ...
    glass = mix(glass, vec3(0.24, 0.001, 0.004), 0.78);
    glass += vec3(1.0, 0.035, 0.025) * surfaceIllumination * rim * 0.15;
    glass += vec3(0.11, 0.001, 0.006) * (1.0 - rim);
    color = mix(color, glass, centerMask * 0.68);

    color += vec3(1.0, 0.01, 0.025) * outerGlow * 0.22 * uEdgeGlow;
    color += vec3(1.0, 0.035, 0.025) * rim * (0.24 + surfaceIllumination * 0.48) * uEdgeGlow;
...
function updateLogoRect(): void {
  ...
  for (const light of lights) light.target.set(0.5, params.logoY + 0.105);
}
