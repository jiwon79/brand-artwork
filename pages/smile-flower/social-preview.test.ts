...
const pageUrl = 'https://brand-artwork.vercel.app/pages/smile-flower/';
...
test('social crawlers can read the Smile Flower card without JavaScript', () => {
  expect(html).toContain(`<link rel="canonical" href="${pageUrl}"`);
  expect(meta('og:type')).toBe('website');
  expect(meta('og:url')).toBe(pageUrl);
  expect(meta('og:title')).toBe('Smile Flower');
