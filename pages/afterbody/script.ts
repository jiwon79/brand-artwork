...
const runtime = new AfterbodyRuntime();
...
async function initializeAfterbody(): Promise<void> {
  ...
  debugWindow.__afterbody = interaction.debugApi();
  cancelAnimationFrame(animationFrame);
  animationFrame = requestAnimationFrame(renderFrame);
}

initializeAfterbody().catch((reason: unknown) => {
  showError(reason instanceof Error ? reason.message : 'Afterbody could not start.');
});
